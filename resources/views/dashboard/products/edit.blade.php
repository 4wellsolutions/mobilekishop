          ...
          <div class="row">
              ...
              <div class="col-12 col-md-6">
                  <div class="form-group">
                      ...
                      <select name="colors[]" id="colors" class="form-control" style="height: 100px;" multiple required>
                  ...
                  <div id="colorImageUploadContainer">
                      @foreach($product->colors as $color)
                          <div class="form-group col-12 col-md-6">
                              ...
                              <input type="file" name="color_images[{{ $color->id }}]" id="color_images_{{ $color->id }}" accept="image/*" class="form-control">
                              @if($color->pivot->image)
                                  <img src="{{ $color->pivot->image }}" alt="{{ $color->name }}" width="100" class="img-thumbnail mt-2">
                              @endif
                          </div>
                      @endforeach
                  </div>
                                  ...
                                  <div class="country-prices row" data-variant-id="{{ $variant->id }}">
                                      @foreach($countries as $country)
                                          <div class="form-group col-6 col-md-3">
                                              <label for="price_{{ $variant->id }}_{{ $country->id }}">{{ $country->country_name }}</label>
                                              <input type="text" name="prices[{{ $variant->id }}][{{ $country->id }}]" id="price_{{ $variant->id }}_{{ $country->id }}" value="{{ old('prices.' . $variant->id . '.' . $country->id, $variantPrices[$variant->id][$country->id] ?? 0) }}" class="form-control" required>
                                          </div>
                                      @endforeach
                                  </div>
                              </div>
              ...
              <div class="form-group col-12 col-md-6">
                  ...
                  <input type="date" name="release_date" id="release_date" value="{{ old('release_date', $product->release_date ? $product->release_date->format('Y-m-d') : '') }}" class="form-control">
              ...
              @if($attributes = $category->attributes()
                  ...
                  @foreach($attributes as $attribute)
                      <div class="form-group col-12 col-md-6">
                          ...
                          <input type="text" name="{{ $attribute->id }}" id="attribute_{{ $attribute->id }}" value="{{ old($attribute->id, $product->attributes->where('id', $attribute->id)->first()->pivot->value ?? '') }}" class="form-control">
                      </div>
                  @endforeach
              @endif
              ...
              <div class="col-12">
                  <div class="form-group">
                      ...
                      <textarea class="form-control" name="body" id="editor" rows="5">{{ old('body', $product->body) }}</textarea>
                  </div>
              </div>
              ...
              <div class="col-12 col-md-6">
                  <div id="imageInputContainer" class="form-group">
                      ...
                      @foreach($product->images as $image)
                          <div class="mb-3 input-group">
                              <img src="{{ url('/products/'.$image->name) }}" alt="{{ $image->alt }}" width="100" class="img-thumbnail">
                              <input type="file" class="form-control image-input" name="images[]" accept="image/*" multiple>
