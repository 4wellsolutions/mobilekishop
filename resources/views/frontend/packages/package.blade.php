                    ...
                    <div class="row g-3">
                        @foreach($packages as $package)
                            <div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xxl-3 mt-4">
                                <a href="{{ route('package.show', [$package->filter_network, $package->slug]) }}" class="text-decoration-none text-dark">
                                <div class="card mb-2 h-100 text-center border-0 shadow">
                                    <img src="{{ URL::to('/images/packages/'.$package->filter_network.'.png') }}" class="img-fluid mx-auto py-2" alt="{{ $package->filter_network }}" style="max-width: 150px;">
                                    <div class="card-body py-1 px-2">
                                        <h2 class="card-title fs-5 fw-normal">{{ $package->name }}</h2>
                                        <p class="card-text">
                                            <small class="text-muted"><strong>Onnet:</strong> {{ $package->onnet }}</small> | 
                                            <small class="text-muted"><strong>Offnet:</strong> {{ $package->offnet }}</small> | 
                                            <small class="text-muted"><strong>SMS:</strong> {{ $package->sms }}</small> | 
                                            <small class="text-muted"><strong>Data:</strong> {{ $package->data }}</small>
                                        </p>
                                        <p class="card-text">
                                            <small class="text-muted"><strong>Validity:</strong> {{ Str::title($package->validity) }}</small> | 
                                            <small class="text-muted"><strong>Price:</strong> Rs.{{ $package->price }} (Incl. Tax)</small>
                                        </p>
