...
@php
    ...
    // Summary variables
    $pixels = $attributes->get('pixels')->pivot->value ?? '';
    $battery = $attributes->get('battery')->pivot->value ?? '';
    $ram_in_gb = $attributes->get('ram_in_gb')->pivot->value ?? '';
    $rom_in_gb = $attributes->get('rom_in_gb')->pivot->value ?? '';

    // Specification variables
    $model = $attributes->get('model')->pivot->value ?? null;
    $ratings = $attributes->get('ratings')->pivot->value ?? null;
    ...
    // Parse the release date attribute if it exists
          ...
          [
            {
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "{{Str::title($product->name)}} was released on {{$release_date}}."
              }
            },{
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "The {{Str::title($product->name)}} features {{$main}} main and {{$front}} selfie cameras"
                }
              },{
              "@type": "Question",
